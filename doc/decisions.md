...
Bootstrap maintains `doc/threat_model.md` as the current security model and uses
STRIDE as the primary structured threat-enumeration method while allowing
project-specific abuse and supply-chain analysis.  The model records assets,
actors, trust assumptions and boundaries, stable `TM-NNN` threats, project and
operator controls, residual risk, evidence, and review triggers; Mermaid diagrams
are preferred when they materially improve understanding.  Security-relevant
architecture and dependency changes trigger review of the current model without
requiring synthetic churn when the existing analysis already covers the changed
boundary.

Source: [ADR-054](adr/ADR-054-Maintain-a-STRIDE-Based-Threat-Model.md)

## ADR-055: Publish Ephemeral ADR Navigation as the Reference Landing Page

**Status:** Accepted

Reference documentation assembles maintained `doc/adr/README.intro.md` and
`README.outro.md` around a linked ADR list generated by the pinned released
`adrctl` dependency.  The composite `doc/adr/README.md` and Doxygen HTML are
ignored generated state, while `make docs` remains offline and non-repairing after
explicit dependency preparation under ADR-051.  Doxygen uses the generated ADR
page as its main page and expands only into the ADR corpus, preserving the
separately maintained STRIDE threat model and its Mermaid diagrams; routine ADR
