...
#
# @details
# Runtime output is part of the user experience.  This module keeps log formatting
# centralized so informational messages, warnings, and errors use the same
# visible shape across the bootstrap engine.
#
# The helpers deliberately keep the first implementation small.  Informational
# messages are suppressed by --quiet because they are non-essential.  Warning and
# error messages are never suppressed here because they explain conservative
# stops, unsupported behavior, or conditions that require user attention.
#
...
# @endcode
#
# Keeping the program name and level stable makes terminal output easier to scan
# and gives future documentation examples a single format to teach.
###############################################################################

###############################################################################
# @fn bootstrap_log_emit(level, message, stream)
...
# this helper so callers can silence progress or placeholder messages without
# suppressing usage errors or other necessary diagnostics.
#
...
# engine from safely continuing.  Quiet mode must not suppress errors because the
# user still needs to know what failed.
#
